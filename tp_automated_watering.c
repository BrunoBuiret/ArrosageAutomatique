...
int main(int argc, char **argv)
{
    ...
    while(isRunning)
    {
        ...
        for(i = 0, j = automaton_get_zones_number(a); i < j; i++)
        {
            ...
            {
                // Log action
                print_log("Turning off lamp #%u.", i + 1);
            ...
            {
                // Log action
                print_log("Turning on lamp #%u.", i + 1);
            ...
            if(is_file(valvePath))
            {
                ...
                if(automaton_read_water_level(a) == LOW)
                {
                    ...
                    if(handle != NULL)
                    {
                        ...
                        do
                        {
                            print_log("Water volume: %u / %u", currentWaterVolume, wantedWaterVolume);
    ...
    print_log("Destroying automaton.");
    automaton_destroy(&a);
